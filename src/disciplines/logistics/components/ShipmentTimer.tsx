...
const CookingTimer: React.FC<CookingTimerProps> = ({ servingSize, setServingSize }) => {
  ...
  const presetTimers = [
    { label: 'Load Plan', minutes: 8 },
    { label: 'Route Build', minutes: 12 },
    { label: 'Dock Check-In', minutes: 6 },
    { label: 'Carrier Handoff', minutes: 10 },
    { label: 'Final Delivery Confirm', minutes: 4 },
  ];
  ...
  const addTimer = (minutes: number, label: string) => {
    ...
    const newTimer: Timer = {
      ...
      label: `${label} (${servingSize} dispatchers)`,
    ...
    <div className="space-y-4">
      ...
      <div className="bg-sand p-4 rounded-lg border border-black">
        ...
        <div className="text-xs text-gray-500">
          *1 dispatcher represents one learner assigned to the run.
        </div>
        <div className="text-xs text-gray-400 mt-1">
          Time recommendations scale by dispatch team size for realistic delivery pacing.
        </div>
      </div>
