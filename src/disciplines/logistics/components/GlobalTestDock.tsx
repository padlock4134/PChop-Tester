...
const GlobalTestDock: React.FC<GlobalTestDockProps> = ({ showcaseRoute }) => {
  ...
  const handleReport = async (sessionId?: string, reason?: string) => {
    ...
    const reportReason = reason || prompt(t('dispatchLounge.globalTestKitchen.describeIssue')) || 'General report';
    
    if (confirm(t('dispatchLounge.globalTestKitchen.reportSession'))) {
      try {
    ...
    if (scheduledDishName.trim() && scheduledCuisine && scheduledDescription.trim() && scheduledDate && scheduledTime) {
      try {
        ...
        const newSession: UpcomingSession = {
          id: data.id,
          hostName: t('dispatchLounge.globalTestKitchen.you'),
    ...
    <div className="bg-white rounded-lg shadow-lg border-4 border-maineBlue overflow-hidden w-full min-h-[800px]">
      <div className="p-4 bg-red-500 text-white font-retro text-center">
        <h2 className="text-xl flex items-center justify-center">
          ...
          {t('dispatchLounge.globalTestKitchen.title')}
        </h2>
      </div>
      
      <div className="p-4">
        <p className="text-sm text-gray-600 text-center mb-4">
          {t('dispatchLounge.globalTestKitchen.subtitle')}
        </p>
        ...
        <div className="flex mb-4 bg-gray-100 rounded-lg p-1">
          ...
          >
            🔴 {t('dispatchLounge.globalTestKitchen.live')}
          </button>
          <button
          ...
          >
            📅 {t('dispatchLounge.globalTestKitchen.upcoming')}
          </button>
          <button
          ...
          >
            🎥 {t('dispatchLounge.globalTestKitchen.host')}
          </button>
        </div>
        ...
        <div className="space-y-3">
          {liveSessions.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              ...
              <p>{t('dispatchLounge.globalTestKitchen.noLiveSessions')}</p>
              <p className="text-sm">{t('dispatchLounge.globalTestKitchen.checkBackLater')}</p>
            </div>
          ) : (
                ...
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600">{t('dispatchLounge.globalTestKitchen.by')} {session.hostName}</p>
                    </div>
                  </div>
                  <div className={`flex items-center text-xs ${session.isEnded ? 'text-gray-500' : 'text-red-600'}`}>
                    {session.isEnded ? (
                      <>
                        ...
                        {t('dispatchLounge.globalTestKitchen.ended')}
                      </>
                    ) : (
                      <>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex items-center text-xs text-gray-500">
                      ...
                      {session.viewers} {t('dispatchLounge.globalTestKitchen.watching')}
                    </div>
                    <button
                    ...
                    >
                      {session.isEnded ? t('dispatchLounge.globalTestKitchen.ended') : t('dispatchLounge.globalTestKitchen.join')}
                    </button>
                  </div>
                </div>
              </div>
          ...
          {upcomingSessions.map((session) => (
            <div key={session.id} className="border border-gray-200 rounded-lg p-3">
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  {session.sessionType && (
                    <span className="text-xs bg-maineBlue text-white px-2 py-0.5 rounded-full">
                      {session.sessionType === 'practice' && `🎯 ${t('dispatchLounge.globalTestKitchen.practice')}`}
                      {session.sessionType === 'assignment' && `📚 ${t('dispatchLounge.globalTestKitchen.assignment')}`}
                      {session.sessionType === 'demo' && `👨‍🏫 ${t('dispatchLounge.globalTestKitchen.demo')}`}
                      {session.sessionType === 'showcase' && `🏆 ${t('dispatchLounge.globalTestKitchen.showcase')}`}
                    </span>
                  )}
                </div>
                <button 
                ...
                >
                  📅 {t('dispatchLounge.globalTestKitchen.addToCalendar')}
                </button>
              </div>
              {session.teacherTag && (
                <div className="text-xs text-gray-600 mb-1">
                  👨‍🏫 {t('dispatchLounge.globalTestKitchen.for')} {session.teacherTag}
                </div>
              )}
            </div>
          ))}
          ...
          <div className="bg-white rounded-lg shadow-lg border-4 border-black p-6 max-w-lg w-full mx-4 relative">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                ...
                >
                  {t('dispatchLounge.globalTestKitchen.cancel')}
                </button>
                <button
                  onClick={() => {
                ...
                >
                  🔴 {t('dispatchLounge.globalTestKitchen.startRecording')}
                </button>
              </div>
            </div>
          </div>
