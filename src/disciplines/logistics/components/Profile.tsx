...
const Profile = () => {
  ...
  const getLocalizedLevelMeta = (level: number) => {
    ...
    return {
      title: t(`levels.logistics.titles.${level}`, { defaultValue: title }),
      icon
    };
  };
  ...
  const generatePDF = async (reportTitle: string) => {
    ...
    if (fileName === 'skill-mastery-tracking') {
      const line1Width = doc.getTextWidth(cleanText('This report analyzes student skill development across core logistics competencies.'));
      doc.text(cleanText('This report analyzes student skill development across core logistics competencies.'), 105 - line1Width/2, 97);
      ...
      const line3Width = doc.getTextWidth(cleanText('and learning outcomes from real-time logistics instruction.'));
      doc.text(cleanText('and learning outcomes from real-time logistics instruction.'), 105 - line3Width/2, 111);
    } else {
      const line1Width = doc.getTextWidth(cleanText('This report provides detailed analytics and insights for logistics education'));
      doc.text(cleanText('This report provides detailed analytics and insights for logistics education'), 105 - line1Width/2, 97);
          ...
          <div className="bg-white rounded-lg shadow-lg border-4 border-black p-6 max-w-4xl w-full mx-4 max-h-[85vh] lg:max-h-[80vh] overflow-y-auto relative">
            ...
            <div className="text-center">
              <p className="text-gray-600 mb-4">
                Master the techniques shown in this tutorial to unlock your full logistics potential!
              </p>
              <button
                onClick={() => {
                  setTutorialModalOpen(false);
