...
const CookingTimer: React.FC<CookingTimerProps> = ({ servingSize, setServingSize }) => {
  ...
  const presetTimers = [
    { label: 'Frame Layout', minutes: 8 },
    { label: 'Concrete Cure Check', minutes: 12 },
    { label: 'Wall Raise', minutes: 6 },
    { label: 'Roof Tie-In', minutes: 10 },
    { label: 'Punch List', minutes: 4 },
  ];
  ...
  const addTimer = (minutes: number, label: string) => {
    ...
    const newTimer: Timer = {
      ...
      label: `${label} (${servingSize} crew members)`,
    ...
    <div className="space-y-4">
      ...
      <div className="bg-sand p-4 rounded-lg border border-black">
        ...
        <div className="text-xs text-gray-500">
          *1 crew member represents one active learner on site.
        </div>
        <div className="text-xs text-gray-400 mt-1">
          Time recommendations scale by crew size for realistic field pacing.
        </div>
      </div>
