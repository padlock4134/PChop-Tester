...
const GlobalTestKitchen: React.FC<GlobalTestKitchenProps> = ({ showcaseRecipe }) => {
  ...
  const handleReport = async (sessionId?: string, reason?: string) => {
    if (!user) {
      alert(t('hardhatHub.globalTestKitchen.pleaseLogInReport'));
      return;
    }
    ...
    if (confirm(t('hardhatHub.globalTestKitchen.reportSession'))) {
      try {
        ...
        if (error) {
          ...
          alert(t('hardhatHub.globalTestKitchen.failedToReport'));
        } else {
          ...
          alert(t('hardhatHub.globalTestKitchen.thankYouReport'));
        }
      } catch (error) {
  ...
  const handleScheduleSession = async () => {
    if (!user) {
      alert(t('hardhatHub.globalTestKitchen.pleaseLogInSchedule'));
      return;
    }
    ...
    if (scheduledDishName.trim() && scheduledCuisine && scheduledDescription.trim() && scheduledDate && scheduledTime) {
      try {
        ...
        if (error) {
          ...
          alert(t('hardhatHub.globalTestKitchen.failedToSchedule').replace('{error}', error.message));
          return;
        }
        ...
        alert(t('hardhatHub.globalTestKitchen.failedToScheduleGeneric'));
      }
    }
  };
  ...
  const handleSaveSession = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        alert(t('hardhatHub.globalTestKitchen.failedToSaveVideo').replace('{error}', error.message || 'Unknown error'));
        setIsSaving(false);
        return;
      }
      ...
      alert(t('hardhatHub.globalTestKitchen.failedToSaveVideoGeneric'));
    } finally {
      setIsSaving(false);
      setSaveConfirmModalOpen(false);
      endRecordingSession();
    ...
    <div className="bg-white rounded-lg shadow-lg border-4 border-maineBlue overflow-hidden w-full min-h-[800px]">
      ...
      <div className="p-4">
        <p className="text-sm text-gray-600 text-center mb-4">
          {t('hardhatHub.globalTestKitchen.subtitle')}
        </p>
