...
const GlobalTestKitchen: React.FC<GlobalTestKitchenProps> = ({ showcaseRecipe }) => {
  ...
  const handleReport = async (sessionId?: string, reason?: string) => {
    if (!user) {
      console.error(t('hardhatHub.globalTestKitchen.pleaseLogInReport'));
      return;
    }

    const reportReason = reason || prompt(t('hardhatHub.globalTestKitchen.describeIssue')) || 'General report';
    
    if (confirm(t('hardhatHub.globalTestKitchen.reportSession'))) {
      try {
        ...
        if (error) {
          ...
          console.error(t('hardhatHub.globalTestKitchen.failedToReport'));
        } else {
          ...
          console.log(t('hardhatHub.globalTestKitchen.thankYouReport'));
        }
      } catch (error) {
  ...
  const handleScheduleSession = async () => {
    if (!user) {
      console.error(t('hardhatHub.globalTestKitchen.pleaseLogInSchedule'));
      return;
    }
    ...
    if (scheduledDishName.trim() && scheduledCuisine && scheduledDescription.trim() && scheduledDate && scheduledTime) {
      try {
        ...
        if (error) {
          ...
          console.error(t('hardhatHub.globalTestKitchen.failedToSchedule').replace('{error}', error.message));
          return;
        }
        ...
        const newSession: UpcomingSession = {
          id: data.id,
          hostName: t('hardhatHub.globalTestKitchen.you'),
        ...
        console.error(t('hardhatHub.globalTestKitchen.failedToScheduleGeneric'));
      }
    }
  };
    ...
    const loadScheduledSessions = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .select('*')
          .eq('user_id', user.id)
          .eq('discipline_slug', 'construction');
  ...
  const handleSaveSession = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        console.error(t('hardhatHub.globalTestKitchen.failedToSaveVideo').replace('{error}', error.message || 'Unknown error'));
        setIsSaving(false);
        return;
      }
      ...
      console.error(t('hardhatHub.globalTestKitchen.failedToSaveVideoGeneric'));
    } finally {
      setIsSaving(false);
      setSaveConfirmModalOpen(false);
      endRecordingSession();
    ...
    <div className="bg-white rounded-lg shadow-lg border-4 border-maineBlue overflow-hidden w-full h-full lg:min-h-[620px] flex flex-col">
      <div className="p-4 bg-red-500 text-white font-retro text-center">
        <h2 className="text-xl flex items-center justify-center">
          ...
          {t('hardhatHub.globalTestKitchen.title')}
        </h2>
      </div>
      ...
      <div className="p-4 flex-1 min-h-0 flex flex-col">
        ...
        <div className="flex mb-4 bg-gray-100 rounded-lg p-1">
          ...
          >
            🔴 {t('hardhatHub.globalTestKitchen.live')}
          </button>
          <button
          ...
          >
            📅 {t('hardhatHub.globalTestKitchen.upcoming')}
          </button>
          <button
          ...
          >
            🎥 {t('hardhatHub.globalTestKitchen.host')}
          </button>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto pr-1 space-y-3">
          {liveSessions.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              ...
              <p>{t('hardhatHub.globalTestKitchen.noLiveSessions')}</p>
              <p className="text-sm">{t('hardhatHub.globalTestKitchen.checkBackLater')}</p>
            </div>
          ) : (
                ...
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600">{t('hardhatHub.globalTestKitchen.by')} {session.hostName}</p>
                    </div>
                  </div>
                  <div className={`flex items-center text-xs ${session.isEnded ? 'text-gray-500' : 'text-red-600'}`}>
                    {session.isEnded ? (
                      <>
                        ...
                        {t('hardhatHub.globalTestKitchen.ended')}
                      </>
                    ) : (
                      <>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex items-center text-xs text-gray-500">
                      ...
                      {session.viewers} {t('hardhatHub.globalTestKitchen.watching')}
                    </div>
                    <button
                    ...
                    >
                      {session.isEnded ? t('hardhatHub.globalTestKitchen.ended') : t('hardhatHub.globalTestKitchen.join')}
                    </button>
                  </div>
                </div>
              </div>
          ...
          {upcomingSessions.map((session) => (
            <div key={session.id} className="border border-gray-200 rounded-lg p-3">
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  {session.sessionType && (
                    <span className="text-xs bg-maineBlue text-white px-2 py-0.5 rounded-full">
                      {session.sessionType === 'practice' && `🎯 ${t('hardhatHub.globalTestKitchen.practice')}`}
                      {session.sessionType === 'assignment' && `📚 ${t('hardhatHub.globalTestKitchen.assignment')}`}
                      {session.sessionType === 'demo' && `👨‍🏫 ${t('hardhatHub.globalTestKitchen.demo')}`}
                      {session.sessionType === 'showcase' && `🏆 ${t('hardhatHub.globalTestKitchen.showcase')}`}
                    </span>
                  )}
                </div>
                <button 
                ...
                >
                  📅 {t('hardhatHub.globalTestKitchen.addToCalendar')}
                </button>
              </div>
              {session.teacherTag && (
                <div className="text-xs text-gray-600 mb-1">
                  👨‍🏫 {t('hardhatHub.globalTestKitchen.for')} {session.teacherTag}
                </div>
              )}
            </div>
          ))}
          ...
          <div className="bg-white rounded-lg shadow-lg border-4 border-black p-6 max-w-lg w-full mx-4 relative">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-4">
                ...
                >
                  {t('hardhatHub.globalTestKitchen.cancel')}
                </button>
                <button
                  onClick={() => {
                ...
                >
                  🔴 {t('hardhatHub.globalTestKitchen.startRecording')}
                </button>
              </div>
            </div>
          </div>
