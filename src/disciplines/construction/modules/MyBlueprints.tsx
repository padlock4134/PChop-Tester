...
const constructionQuoteOfTheDay = {
  chef: 'Norm Abram',
  quote: 'Measure carefully, build confidently, and let quality speak for itself.'
};

export function getChefQuoteOfTheDay() {
  return constructionQuoteOfTheDay;
}

export function getVideoQueriesForRecipe(recipe: Recipe): string[] {
  return [
    `how to ${recipe.name} construction tutorial`,
    `${recipe.name} building guide`
  ];
}
...
const MyBlueprints = () => {
  ...
  */];
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from('user_collections').select('*').eq('user_id', user.id).eq('discipline', 'construction')
      .then(({ data }) => {
  ...
  const handleCreateCollection = async () => {
    ...
    const { data, error } = await supabase.from('user_collections').insert({
      user_id: user?.id, discipline: 'construction', name: newCollectionName.trim(), emoji: '📁', items: selectedRecipes
    }).select().single();
    if (!error && data) {
    ...
    try {
      ...
      switch (platform) {
        ...
        case 'instagram':
          ...
          console.log(t('myBlueprints.sharingInstructions'));
          shared = true;
          break;
        case 'slack':
        ...
        case 'native':
          if (navigator.share) {
            ...
            console.log(t('myBlueprints.linkCopied'));
            shared = true;
          }
          break;
      }
      ...
      if (err.name !== 'AbortError') {
        console.error(t('myBlueprints.failedToShare'));
      }
    } finally {
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:h-full lg:justify-center">
        <div className={`lg:w-[66.666%] bg-weatheredWhite rounded-xl shadow-lg border-4 border-maineBlue flex flex-col h-full lg:min-h-[620px] ${
          ...
          <div className="flex items-center justify-center p-6 pb-4">
            ...
            <h1 className="text-3xl font-retro text-maineBlue mb-0">{t('myBlueprints.title')}</h1>
          </div>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => {
        ...
        <div className="bg-white p-6 rounded-lg shadow-xl max-w-sm w-full mx-4 border-4 border-black" onClick={e => e.stopPropagation()}>
          <h3 className="text-lg font-bold mb-4">
            {recipeToShare ? `${t('myBlueprints.shareRecipeTitle')} "${recipeToShare.name}"` : t('myBlueprints.shareYourCookbook')}
          </h3>
          <div className="flex justify-around mb-4">
            <button 
              onClick={() => handleShare('facebook')}
              className="p-2 rounded-full hover:bg-blue-100"
              title={t('myBlueprints.shareOnFacebook')}
            >
            ...
            <button 
              ...
              className="p-2 rounded-full hover:bg-blue-100"
              title={t('myBlueprints.shareOnTwitter')}
            >
            ...
            <button 
              ...
              title={t('myBlueprints.shareOnPinterest')}
            >
            ...
            <button 
              ...
              title={t('myBlueprints.shareOnWhatsApp')}
            >
            ...
            <button 
              ...
              title={t('myBlueprints.shareOnInstagram')}
            >
            ...
            <button 
              ...
              title={t('myBlueprints.shareOnSlack')}
            >
      ...
      {showCreateCollectionModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => {
          ...
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full mx-4 border-4 border-black" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4">{t('myBlueprints.createNewCollection')}</h3>
            
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                {t('myBlueprints.collectionName')}
              </label>
              <input
                type="text"
                value={newCollectionName}
                onChange={(e) => setNewCollectionName(e.target.value)}
                placeholder={t('myBlueprints.enterCollectionName')}
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-600">
                {t('myBlueprints.selectedRecipes')} {selectedRecipes.length}
              </p>
            </div>
            ...
            <div className="flex justify-end gap-3">
              ...
              >
                {t('myBlueprints.createCollection')}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => {
          ...
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full mx-4 border-4 border-black" onClick={e => e.stopPropagation()}>
              ...
              <div className="h-40 overflow-y-scroll border border-gray-300 rounded p-2">
                ...
                {selectedCollection.recipes.length === 0 && (
                  <div className="py-4 text-center text-gray-500 text-sm italic">
                    {t('myBlueprints.noRecipesInCollection')}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex justify-between">
              ...
              >
                {t('myBlueprints.deleteCollection')}
              </button>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-3">
          ...
          <div className="relative flex-1">
            <input
              type="text"
              placeholder={t('myBlueprints.searchRecipes')}
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-1 w-full">
          ...
          >
            <span className="hidden sm:inline">{t('myBlueprints.previous')}</span>
            <span className="sm:hidden">{t('myBlueprints.prev')}</span>
          </button>
          <button
          ...
          >
            <span className="hidden sm:inline">{t('myBlueprints.next')}</span>
            <span className="sm:hidden">{t('myBlueprints.next')} →</span>
          </button>
          <button
            onClick={async () => {
            ...
            title={t('myBlueprints.deleteRecipe')}
          >
            🗑️ {t('myBlueprints.remove')}
          </button>
          <button
            onClick={() => {
            ...
            >
                ...
                <div className="flex-1 flex flex-col justify-center w-full px-2">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 w-full">
                    ...
                    <div className="bg-seafoam/20 p-1.5 rounded-lg text-center border-2 border-seafoam">
                      <h4 className="font-bold mb-0.5 text-xs sm:text-sm text-maineBlue">🥘 {t('myBlueprints.ingredients')}</h4>
                      <ul className="list-disc pl-3 max-h-[50px] sm:max-h-[60px] overflow-y-auto text-left text-[10px] sm:text-xs space-y-0">
                        ...
                        {(filteredRecipes[currentIndex].ingredients?.length || 0) > 6 && (
                          <li className="text-gray-600 italic font-semibold">+{(filteredRecipes[currentIndex].ingredients?.length || 0) - 6} {t('myBlueprints.more')}</li>
                        )}
                      </ul>
                    </div>
                    ...
                    <div className="bg-amber-50 p-1.5 rounded-lg text-center border-2 border-amber-300">
                      <h4 className="font-bold mb-0.5 text-xs sm:text-sm text-amber-900">🔪🏗️ {t('myBlueprints.equipment')}</h4>
                      <ul className="list-disc pl-3 max-h-[40px] sm:max-h-[50px] overflow-y-auto text-left text-[10px] sm:text-xs space-y-0">
                        {filteredRecipes[currentIndex].equipment?.slice(0, 3).map((item, i) => (
                          <li key={i} className="line-clamp-1">{item}</li>
                        ))}
                        {(filteredRecipes[currentIndex].equipment?.length || 0) > 3 && (
                          <li className="text-gray-600 italic font-semibold">+{(filteredRecipes[currentIndex].equipment?.length || 0) - 3} {t('myBlueprints.more')}</li>
                        )}
                      </ul>
                    </div>
                    ...
                    <div className="bg-green-50 p-1.5 rounded-lg text-center border-2 border-green-300">
                      <h4 className="font-bold mb-0.5 text-xs sm:text-sm text-green-900">🥗 {t('myBlueprints.healthTags')}</h4>
                      <div className="flex flex-wrap gap-1 justify-center max-h-[50px] sm:max-h-[60px] overflow-y-auto">
                        {filteredRecipes[currentIndex].healthTags?.slice(0, 4).map(tag => (
                          ...
                          <span className="text-xs text-gray-500">{t('myBlueprints.noHealthTags')}</span>
                        )}
              ...
              <div className="absolute inset-0 h-full w-full rounded-lg bg-white p-2 sm:p-3 shadow-lg border-4 border-lobsterRed [transform:rotateY(180deg)] [backface-visibility:hidden] flex flex-col">
                ...
                <div className="flex-grow overflow-y-auto px-1">
                  <h4 className="font-bold mb-1 text-xs sm:text-sm text-maineBlue">📋 {t('myBlueprints.instructions')}</h4>
              ...
              <div className="mb-6">
                ...
                <div className="max-h-32 overflow-y-auto border border-gray-300 rounded p-2 bg-gray-50">
                  ...
                  {collections.length === 0 && (
                    <div className="py-4 text-center text-gray-500 text-sm italic">
                      {t('myBlueprints.noCollectionsYet')}
                    </div>
                  )}
                </div>
              </div>
                  ...
                  <div className="w-full lg:w-1/2 h-1/2 lg:h-full bg-white border-b lg:border-b-0 lg:border-r-2 border-gray-300 rounded-t-lg lg:rounded-t-none lg:rounded-l-lg p-3 lg:p-4 overflow-y-auto flex flex-col">
                    ...
                    <div className="bg-white border-2 lg:border-4 border-green-500 rounded-lg p-2 lg:p-3 mb-2 shadow-sm flex-shrink-0">
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-2">
                        <h4 className="font-serif font-bold text-emerald-800 text-sm lg:text-xs">📊 {t('myBlueprints.rubric')}</h4>
                      <select 
                      ...
                      <div className="grid grid-cols-2 gap-1 lg:gap-1">
                        ...
                        <div className="bg-white/60 p-2 lg:p-1 rounded border border-emerald-200">
                          <div className="text-xs font-medium text-emerald-900 mb-1">{t('myBlueprints.technique')} (25)</div>
                          <select className="w-full text-xs border border-emerald-300 rounded px-1 py-1 lg:py-0.5 bg-white min-h-[32px] lg:min-h-0">
                          <option value="">{t('myBlueprints.score')}</option>
                        ...
                        {/* Safety Score */}
                        <div className="bg-white/60 p-2 lg:p-1 rounded border border-emerald-200">
                          <div className="text-xs font-medium text-emerald-900 mb-1">{t('myBlueprints.safety')} (25)</div>
                          <select className="w-full text-xs border border-emerald-300 rounded px-1 py-1 lg:py-0.5 bg-white min-h-[32px] lg:min-h-0">
                          <option value="">{t('myBlueprints.score')}</option>
                          <option value="25">A (23-25)</option>
                          <option value="22">B (20-22)</option>
                        ...
                        {/* Consistency Score */}
                        <div className="bg-white/60 p-2 lg:p-1 rounded border border-emerald-200">
                          <div className="text-xs font-medium text-emerald-900 mb-1">{t('myBlueprints.consistency')} (25)</div>
                          <select className="w-full text-xs border border-emerald-300 rounded px-1 py-1 lg:py-0.5 bg-white min-h-[32px] lg:min-h-0">
                          <option value="">{t('myBlueprints.score')}</option>
                          <option value="25">A (23-25)</option>
                          <option value="22">B (20-22)</option>
                        ...
                        {/* Presentation Score */}
                        <div className="bg-white/60 p-2 lg:p-1 rounded border border-emerald-200">
                          <div className="text-xs font-medium text-emerald-900 mb-1">{t('myBlueprints.presentation')} (25)</div>
                          <select className="w-full text-xs border border-emerald-300 rounded px-1 py-1 lg:py-0.5 bg-white min-h-[32px] lg:min-h-0">
                          <option value="">{t('myBlueprints.score')}</option>
                          <option value="25">A (23-25)</option>
                          <option value="22">B (20-22)</option>
                      ...
                      <div className="mt-2 pt-2 border-t border-emerald-300 text-center py-2 lg:py-3">
                        <span className="text-base lg:text-xl font-bold text-red-600">
                          <span className="block lg:inline">{t('myBlueprints.total')}: {(mockGrades as any)[students[currentStudentIndex].id]?.[assignments[currentAssignmentPage].id]?.total || '--'} / 100</span>
                          <span className="hidden lg:inline"> | </span>
                          <span className="block lg:inline">{t('myBlueprints.grade')}: {(mockGrades as any)[students[currentStudentIndex].id]?.[assignments[currentAssignmentPage].id]?.grade || '--'}</span>
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-white border-2 lg:border-4 border-yellow-500 rounded-lg p-2 lg:p-3 mb-2 shadow-sm flex-shrink-0">
                      <h4 className="font-serif font-bold text-amber-800 mb-1 lg:mb-2 text-sm">💬 {t('myBlueprints.feedback')}</h4>
                      
                      <textarea 
                        placeholder={t('myBlueprints.feedbackPlaceholder')}
                    ...
                    <div className="bg-white border-2 lg:border-4 border-amber-700 rounded-lg p-2 lg:p-3 shadow-sm overflow-hidden flex flex-col flex-1 min-h-0">
                      <h4 className="font-serif font-bold text-yellow-800 mb-2 text-sm border-b border-yellow-300 pb-1">📝 {t('myBlueprints.savedFeedbackNotes')}</h4>
                    ...
                    <div className="flex flex-col bg-white w-full h-full overflow-hidden rounded-lg border-4 border-maineBlue">
                      ...
                      <div className="w-full h-20 lg:h-24 bg-gray-100 flex items-center justify-center border-b-2 border-amber-300 flex-shrink-0">
                        <div className="text-center">
                          ...
                          <div className="text-xs font-bold text-amber-800">{assignments[currentAssignmentPage].week} {t('myBlueprints.assignment')}</div>
                        </div>
                      </div>
                      ...
                      <div className="p-2 lg:p-3 bg-white text-center flex-1 overflow-y-auto flex flex-col">
                        ...
                        <div className="text-xs text-gray-600 mb-2 lg:mb-4">
                          <span className="block lg:inline">{t('myBlueprints.due')}: {assignments[currentAssignmentPage].dueDate}</span>
                          <span className="hidden lg:inline"> | </span>
                        ...
                        <div className="space-y-1 lg:space-y-2 flex-shrink-0">
                          <div>
                            <div className="font-semibold mb-1 text-sm text-amber-800">{t('myBlueprints.requiredTechniques')}</div>
                          ...
                          <div>
                            <div className="font-semibold mb-1 text-sm text-amber-800">{t('myBlueprints.submission')}</div>
                            <div className="text-xs text-gray-700 leading-tight">
                          ...
                          <div>
                            <div className="font-semibold mb-1 text-sm text-amber-800">{t('myBlueprints.objectives')}</div>
                            <div className="text-xs text-gray-700 leading-tight">
                        ...
                        <div className="text-center mt-2 flex-1 flex flex-col min-h-0">
                          <div className="flex flex-col lg:flex-row items-center justify-center gap-1 lg:gap-2 mb-2 flex-shrink-0">
                            <h4 className="font-serif font-semibold text-amber-800 text-sm">{students[currentStudentIndex].name} - {t('myBlueprints.submission')}</h4>
                            <select 
                          ...
                          <div className="bg-gray-900 rounded-lg overflow-hidden border border-amber-300 relative flex-1 min-h-[150px]">
                            <div className="h-full bg-gray-800 flex items-center justify-center">
                              <div className="text-center text-white p-2">
                                ...
                                <div className="text-xs text-gray-300 mt-1 hidden lg:block">{t('myBlueprints.submittedVia')}</div>
                              </div>
                            </div>
                            <button 
              ...
              <div className="p-4 bg-gray-100">
                <div className="bg-black aspect-video flex items-center justify-center border-2 border-gray-400 rounded-lg overflow-hidden shadow-lg">
                  <div className="text-center text-white">
                    ...
                    <div className="text-sm text-gray-300">{t('myBlueprints.clickToPlay')}</div>
                    <div className="text-xs text-gray-400 mt-2">{t('myBlueprints.submittedVia')}</div>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-2xl border-4 border-black w-full max-w-4xl max-h-[90vh] overflow-y-auto flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bg-purple-100 border-b-4 border-purple-400 p-6">
              <div className="flex justify-between items-center mb-4">
                <div>
                  <h2 className="text-3xl font-bold text-purple-800 font-retro">🎥 {t('myBlueprints.myTestKitchenVideos')}</h2>
                  <p className="text-purple-600 mt-1">{t('myBlueprints.reviewSavedVideos')}</p>
                </div>
                <button
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                <div className="flex items-center gap-2">
                  <label className="text-purple-700 font-bold text-sm">{t('myBlueprints.category')}:</label>
                  <select
                    value={videoFilter}
                  ...
                  >
                    <option value="all">{t('myBlueprints.allVideos')}</option>
                    <option value="practice">{t('myBlueprints.practiceSessions')}</option>
                    <option value="assignments">{t('myBlueprints.assignmentSubmissions')}</option>
                    <option value="demos">{t('myBlueprints.demoRecordings')}</option>
                  </select>
                </div>
                
                <div className="flex items-center gap-2">
                  <label className="text-purple-700 font-bold text-sm">{t('myBlueprints.user')}:</label>
                  <select
                    value={userFilter}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {loadingVideos ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">{t('myBlueprints.loadingYourVideos')}</p>
                </div>
                        ...
                        <div className="flex items-center gap-4 flex-1">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-3 mt-1">
                              ...
                              {video.isPublic ? (
                                <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded font-bold">🌍 {t('myBlueprints.public')}</span>
                              ) : (
                                <span className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded font-bold">🔒 {t('myBlueprints.private')}</span>
                              )}
                            </div>
