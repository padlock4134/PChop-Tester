...
const GlobalTestKitchen: React.FC<GlobalTestKitchenProps> = ({ showcaseRecipe }) => {
  ...
  const handleReport = async (sessionId?: string, reason?: string) => {
    if (!user) {
      console.error(t('wireLounge.globalTestKitchen.pleaseLogInReport'));
      return;
    }
    ...
    if (confirm(t('wireLounge.globalTestKitchen.reportSession'))) {
      try {
        ...
        if (error) {
          ...
          console.error(t('wireLounge.globalTestKitchen.failedToReport'));
        } else {
          ...
          console.log(t('wireLounge.globalTestKitchen.thankYouReport'));
        }
      } catch (error) {
  ...
  const handleScheduleSession = async () => {
    if (!user) {
      console.error(t('wireLounge.globalTestKitchen.pleaseLogInSchedule'));
      return;
    }
    ...
    if (scheduledDishName.trim() && scheduledCuisine && scheduledDescription.trim() && scheduledDate && scheduledTime) {
      try {
        ...
        if (error) {
          ...
          console.error(t('wireLounge.globalTestKitchen.failedToSchedule').replace('{error}', error.message));
          return;
        }
        ...
        console.error(t('wireLounge.globalTestKitchen.failedToScheduleGeneric'));
      }
    }
  };
  ...
  const handleSaveSession = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        console.error(t('wireLounge.globalTestKitchen.failedToSaveVideo').replace('{error}', error.message || 'Unknown error'));
        setIsSaving(false);
        return;
      }
      ...
      console.error(t('wireLounge.globalTestKitchen.failedToSaveVideoGeneric'));
    } finally {
      setIsSaving(false);
      setSaveConfirmModalOpen(false);
      endRecordingSession();
