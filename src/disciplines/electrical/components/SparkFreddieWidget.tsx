...
const getProactiveMessage = (page: string, t: any) => {
  switch (page) {
    case 'MyKitchen':
      return t('sparkFreddie.welcomeMyKitchen');
    case 'MyCookBook':
      return t('sparkFreddie.welcomeMyCookbook');
    case 'ChefsCorner':
      return t('sparkFreddie.welcomeChefsCorner');
    case 'CulinarySchool':
      return t('sparkFreddie.welcomeCulinarySchool');
    default:
      return t('sparkFreddie.welcomeDefault');
  }
};
  ...
  return (
    <>
      <button
        ...
        onClick={() => setOpen(o => !o)}
        aria-label="Open Spark Freddie AI Assistant"
      >
        <img
          src={chefFreddiePng}
          alt="Spark Freddie"
        ...
        <React.Fragment>
          <div className="fixed bottom-24 right-6 bg-white border-4 border-maineBlue rounded shadow-lg p-4 w-80 z-50 flex flex-col max-h-[60vh]">
            <div className="flex justify-between items-center mb-2">
              <span className="font-bold text-maineBlue">Spark Freddie</span>
              <button onClick={() => {
                setOpen(false);
                setMessages([]);
