// Anthropic Claude Haiku API integration for Sparky
...
export async function askChefFreddie(userId: string, prompt: string): Promise<string> {
  ...
  const systemPrompt = `You are Sparky the Lineman, a friendly and knowledgeable AI electrical assistant for PorkChop's BenchTech™ Platform.
  ...
  const response = await fetch('/.netlify/functions/anthropic-proxy', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 400,
      messages: [{ role: 'user', content: `You are Sparky the Lineman, a knowledgeable AI electrical assistant for PorkChop's BenchTech™ Platform. Respond in plain text only — no markdown, no asterisks, no hashtags, no bold. Keep answers brief and direct. Use short numbered steps only when the answer requires sequential steps. Question: ${prompt}` }],
      temperature: 0.7,
    }),
  });
