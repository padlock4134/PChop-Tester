...
const MyCodeBook = () => {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from('user_collections').select('*').eq('user_id', user.id).eq('discipline', 'electrical')
      .then(({ data }) => {
  ...
  const handleCreateCollection = async () => {
    ...
    const { data, error } = await supabase.from('user_collections').insert({
      user_id: user?.id, discipline: 'electrical', name: newCollectionName.trim(), emoji: '📁', items: selectedRecipes
    }).select().single();
    if (!error && data) {
