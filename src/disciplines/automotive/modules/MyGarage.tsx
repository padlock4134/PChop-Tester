...
function categorizeIngredient(name: string): string {
  ...
  if (/(tire|wheel|rim|rubber|tread|puncture|flat)/.test(n)) return "Tire";
  if (/(brake|pad|rotor|caliper|disc|drum|abs|braking)/.test(n)) return "Brake";
  if (/(shock|strut|spring|suspension|control arm|bushing|ball joint|tie rod)/.test(n)) return "Suspension";
  if (/(engine|motor|piston|cylinder|valve|gasket|oil|filter|spark|timing|camshaft|crankshaft)/.test(n)) return "Engine";
  if (/(transmission|gearbox|clutch|flywheel|driveshaft|differential|axle|cv joint)/.test(n)) return "Transmission";
  if (/(battery|alternator|starter|wiring|fuse|relay|sensor|ecu|electrical|diagnostic)/.test(n)) return "Electrical";
  if (/(bumper|fender|door|hood|trunk|roof|mirror|glass|panel|paint)/.test(n)) return "Body";
  if (/(seat|dashboard|console|carpet|upholstery|trim|interior|steering|radio|speaker)/.test(n)) return "Interior";
  return "Other";
}
...
const MyGarage = () => {
  ...
  return (
    <>
      <div className="w-full bg-white rounded-lg shadow-lg border-4 border-maineBlue flex flex-col max-h-[calc(100vh-100px)] student-dashboard-frame">
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center justify-center">
        ...
        <input
          ...
          onChange={async (e) => {
            ...
            try {
              ...
              setScanError(t('myGarage.failedToScan'));
              setScanLoading(false);
            }
          }}
        />
        ...
        >
          {scanLoading ? t('myGarage.scanning') : t('myGarage.scanKitchen')}
        </button>
        <button
          ...
          onClick={async () => {
            ...
            try {
              ...
              setMatcherError('Failed to fetch repair guides.');
            } finally {
      ...
      {scanLoading && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-weatheredWhite p-8 rounded shadow-lg flex flex-col items-center">
            ...
            <div className="text-lg font-retro mb-2">{t('myGarage.scanningPhoto')}</div>
          </div>
        </div>
      )}
      {scanError && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
          <div className="bg-weatheredWhite p-8 rounded shadow-lg flex flex-col items-center">
            ...
            <button className="bg-lobsterRed text-weatheredWhite px-4 py-2 rounded font-bold mt-2" onClick={() => setScanError('')}>{t('myGarage.close')}</button>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-4 w-full">
        ...
        <input
          ...
          placeholder={t('myGarage.searchCupboard')}
          value={filterText}
        ...
        <input
          type="text"
          className="border px-3 py-2 rounded w-full sm:w-1/3"
          placeholder={t('myGarage.addAnIngredient')}
          value={input}
      ...
      <div className="bg-gradient-to-br from-yellow-100 to-sand border-4 border-yellow-900 rounded-2xl shadow-lg p-4 relative overflow-hidden">
        ...
        {filteredIngredients.length === 0 ? (
          <div className="text-gray-500 italic text-center py-8 relative z-10">{t('myGarage.noMatchingIngredients')}</div>
        ) : (
