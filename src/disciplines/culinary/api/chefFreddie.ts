...
export async function askChefFreddie(userId: string, prompt: string): Promise<string> {
  ...
  const systemPrompt = `You are Tojimaster Kito, a friendly and knowledgeable AI chef assistant for PorkChop's BenchTech™ Platform.
  ...
  const response = await fetch('/.netlify/functions/anthropic-proxy', {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 400,
      messages: [{ role: 'user', content: `You are Tojimaster Kito, a knowledgeable AI chef assistant for PorkChop's BenchTech™ Platform. Respond in plain text only — no markdown, no asterisks, no hashtags, no bold. Keep answers brief and direct. Use short numbered steps only when the answer requires sequential steps. Question: ${prompt}` }],
      temperature: 0.7,
    }),
  });
