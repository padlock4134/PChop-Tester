...
const GlobalTestKitchen: React.FC<GlobalTestKitchenProps> = ({ showcaseRecipe }) => {
  ...
  const handleReport = async (sessionId?: string, reason?: string) => {
    if (!user) {
      console.error(t('chefsCorner.globalTestKitchen.pleaseLogInReport'));
      return;
    }
    ...
    if (confirm(t('chefsCorner.globalTestKitchen.reportSession'))) {
      try {
        ...
        if (error) {
          ...
          console.error(t('chefsCorner.globalTestKitchen.failedToReport'));
        } else {
          ...
          console.log(t('chefsCorner.globalTestKitchen.thankYouReport'));
        }
      } catch (error) {
  ...
  const handleScheduleSession = async () => {
    if (!user) {
      console.error(t('chefsCorner.globalTestKitchen.pleaseLogInSchedule'));
      return;
    }
    ...
    if (scheduledDishName.trim() && scheduledCuisine && scheduledDescription.trim() && scheduledDate && scheduledTime) {
      try {
        ...
        if (error) {
          ...
          console.error(t('chefsCorner.globalTestKitchen.failedToSchedule').replace('{error}', error.message));
          return;
        }
        ...
        console.error(t('chefsCorner.globalTestKitchen.failedToScheduleGeneric'));
      }
    }
  };
  ...
  const handleSaveSession = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        console.error(t('chefsCorner.globalTestKitchen.failedToSaveVideo').replace('{error}', error.message || 'Unknown error'));
        setIsSaving(false);
        return;
      }
      ...
      console.error(t('chefsCorner.globalTestKitchen.failedToSaveVideoGeneric'));
    } finally {
      setIsSaving(false);
      setSaveConfirmModalOpen(false);
      endRecordingSession();
        ...
        <div className="flex-1 min-h-0 overflow-y-auto pr-1 space-y-4">
          ...
          <div className="bg-sand p-3 rounded-lg">
            ...
            <ul className="text-xs text-gray-700 space-y-2">
              ...
              <li className="flex items-center gap-2">
                <span className="text-yellow-500">●</span> Recipe or topic prepared
              </li>
              <li className="flex items-center gap-2">
                <span className="text-yellow-500">●</span> Ingredients/materials ready
              </li>
            </ul>
          </div>
        </div>
