...
import { getVideoQueriesForProject } from './MySpecBook';
import { getTutorialVideo, TutorialVideoResult } from '../utils/videoSearch';

import type { Project } from './MySpecBook';
...
const MySpecBookVideoModals: React.FC<Props> = ({ project, open, onClose }) => {
  ...
  useEffect(() => {
    if (!open) return;
    setLoading(true);
    const queries = getVideoQueriesForProject(project);
    ...
    <div>
      ...
      {!loading && videoUrls.map((url, idx) => (
        <div key={idx} style={{ marginBottom: 24 }}>
          <h3>{idx === 0 ? `How to complete ${project.name}` : `How to prepare primary material`}</h3>
          {url ? (
            <iframe
              width="100%"
              height="315"
...
export default MySpecBookVideoModals;
