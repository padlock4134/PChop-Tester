...
const MySpecBook = () => {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from('user_collections').select('*').eq('user_id', user.id).eq('discipline', 'welding')
      .then(({ data }) => {
        if (data) setCollections(data.map(r => ({ id: r.id, name: r.name, emoji: r.emoji, projects: r.items as string[] })));
      });
  }, [user]);
  ...
  const handleCreateCollection = async () => {
    if (!newCollectionName.trim() || selectedProjectIds.length === 0) return;
    const { data, error } = await supabase.from('user_collections').insert({
      user_id: user?.id, discipline: 'welding', name: newCollectionName.trim(), emoji: '📁', items: selectedProjectIds
    }).select().single();
    if (!error && data) {
      setCollections(prev => [...prev, { id: data.id, name: data.name, emoji: data.emoji, projects: data.items as string[] }]);
    }
    setNewCollectionName(''); setSelectedProjectIds([]); setShowCreateCollectionModal(false);
  };
    ...
    <div className="w-[90%] mx-auto mt-4 student-dashboard-height-lock">
          ...
          >
            ...
            >
              ...
              <div className="absolute inset-0 bg-white p-2 sm:p-3 rounded-lg shadow-lg border-4 border-maineBlue flex flex-col items-center [backface-visibility:hidden]">
                ...
                <h3 className="text-sm sm:text-base md:text-lg font-bold mb-1 text-center text-maineBlue">{filteredProjects[currentIndex].name}</h3>
                <div className="flex-1 flex flex-col justify-center w-full px-2">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 w-full">
                    ...
                    <div className="bg-seafoam/20 p-1.5 rounded-lg text-center border-2 border-seafoam">
                      <h4 className="font-bold mb-0.5 text-xs sm:text-sm text-maineBlue">🥘🔩 {t('mySpecBook.materials', { defaultValue: 'Materials' })}</h4>
                    ...
                    <div className="bg-amber-50 p-1.5 rounded-lg text-center border-2 border-amber-300">
                      <h4 className="font-bold mb-0.5 text-xs sm:text-sm text-amber-900">🔪🧲 {t('mySpecBook.equipment')}</h4>
                      <ul className="list-disc pl-3 max-h-[50px] sm:max-h-[60px] overflow-y-auto text-left text-[10px] sm:text-xs space-y-0">
                    ...
                    <div className="bg-green-50 p-1.5 rounded-lg text-center border-2 border-green-300">
                      <h4 className="font-bold mb-0.5 text-xs sm:text-sm text-green-900">🥗🏅 {t('mySpecBook.skillTags', { defaultValue: 'Skill Tags' })}</h4>
              ...
              <div className="absolute inset-0 h-full w-full rounded-lg bg-white p-2 sm:p-3 shadow-lg border-4 border-lobsterRed [transform:rotateY(180deg)] [backface-visibility:hidden] flex flex-col">
                <h3 className="text-sm sm:text-base md:text-lg font-bold mb-1 text-center text-lobsterRed border-b-2 border-lobsterRed pb-1">{filteredProjects[currentIndex].name}</h3>
                <div className="flex-grow overflow-y-auto px-1">
                  <h4 className="font-bold mb-1 text-xs sm:text-sm text-maineBlue">📋 {t('mySpecBook.instructions')}</h4>
                  <p className="whitespace-pre-wrap text-[10px] sm:text-xs leading-snug">{filteredProjects[currentIndex].instructions}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mt-3 text-center text-sm">
        {(() => {
          ...
          return (
            <span className="italic">"{quoteOfDay.quote}" <span className="not-italic text-gray-600">— {quoteOfDay.pioneer}</span></span>
          );
        })()}
      </div>
        </div>
