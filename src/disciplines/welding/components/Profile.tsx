...
const ClassScheduleModal = ({ open, onClose, onOpenRegistration }: { open: boolean; onClose: () => void; onOpenRegistration: () => void }) => {
  ...
  const currentClasses = [
    { icon: '🛠️', name: 'Manual Mill Operations', instructor: 'Instructor Martinez', time: 'Mon/Wed 9:00 AM' },
    { icon: '📐', name: 'Precision Measurement & Metrology', instructor: 'Instructor Johnson', time: 'Tue/Thu 11:00 AM' },
    { icon: '🧰', name: 'CNC Setup & Tool Offsets', instructor: 'Instructor Williams', time: 'Fri 1:00 PM' }
  ];
...
const Profile = () => {
  ...
  const getLocalizedLevelMeta = (level: number) => {
    ...
    return {
      title: t(`levels.machining.titles.${level}`, { defaultValue: title }),
      icon
    };
  };
  ...
  const talentTrees = {
    'Precision Machinist': [
      { name: 'Setup Precision', icon: FireIcon, unlockLevel: 10, description: 'Improve setup repeatability and first-part success' },
      { name: 'Tolerance Keeper', icon: ShieldCheckIcon, unlockLevel: 14, description: 'Hold tight tolerances with consistent process control' },
    ...
    'CNC Programming Expert': [
      { name: 'Toolpath Architect', icon: FireIcon, unlockLevel: 10, description: 'Create efficient toolpaths with fewer collisions' },
      { name: 'Post Processor Pro', icon: ShieldCheckIcon, unlockLevel: 14, description: 'Tune post outputs for reliable machine execution' },
    ...
    'Metrology Specialist': [
      { name: 'Gauge Mastery', icon: FireIcon, unlockLevel: 10, description: 'Use gauges and comparators with high repeatability' },
      { name: 'CMM Operator', icon: ShieldCheckIcon, unlockLevel: 14, description: 'Run CMM programs for accurate feature verification' },
  ...
  const generatePDF = async (reportTitle: string) => {
    ...
    if (fileName === 'skill-mastery-tracking') {
      const line1Width = doc.getTextWidth(cleanText('This report analyzes student skill development across core machining competencies.'));
      doc.text(cleanText('This report analyzes student skill development across core machining competencies.'), 105 - line1Width/2, 97);
      ...
      const line3Width = doc.getTextWidth(cleanText('and learning outcomes from real-time machining instruction.'));
      doc.text(cleanText('and learning outcomes from real-time machining instruction.'), 105 - line3Width/2, 111);
    } else {
      const line1Width = doc.getTextWidth(cleanText('This report provides detailed analytics and insights for machining education'));
      doc.text(cleanText('This report provides detailed analytics and insights for machining education'), 105 - line1Width/2, 97);
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            ...
            >
              ...
              <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 z-10 hidden group-hover:block bg-white text-black p-2 rounded shadow-lg text-xs w-40 sm:w-48 border border-gray-300">
                ...
                <div className="mt-1">Master precision setup, machining accuracy, and part quality.</div>
              </div>
            </button>
            ...
            >
              ...
              {/* Mobile-friendly tooltip */}
              <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 z-10 hidden group-hover:block bg-white text-black p-2 rounded shadow-lg text-xs w-40 sm:w-48 border border-gray-300">
                <strong>CNC Programming Expert</strong>
                <div className="mt-1">Specialize in toolpaths, feeds/speeds, and cycle optimization.</div>
              </div>
            </button>
            ...
            >
              ...
              {/* Mobile-friendly tooltip */}
              <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 z-10 hidden group-hover:block bg-white text-black p-2 rounded shadow-lg text-xs w-40 sm:w-48 border border-gray-300">
                <strong>Metrology Specialist</strong>
                <div className="mt-1">Lead measurement systems, inspection methods, and quality control.</div>
              </div>
            </button>
          </div>
          )}
          ...
          <div className="bg-white rounded-lg shadow-lg border-4 border-black p-6 max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto relative">
            ...
            <div className="text-center">
              <p className="text-gray-600 mb-4">
                Master the techniques shown in this tutorial to unlock your full machining potential!
              </p>
              <button
                onClick={() => {
                  setTutorialModalOpen(false);
          ...
          <div className="bg-white rounded-lg shadow-lg border-4 border-black p-4 sm:p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
              {talentTrees[selectedTalentTree === 'Equipment' ? 'Precision Machinist' : 
                           selectedTalentTree === 'Techniques' ? 'CNC Programming Expert' : 'Metrology Specialist']?.map(talent => {
