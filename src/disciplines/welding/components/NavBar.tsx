    ...
    <button
      onClick={() => {
        if (isOnAdmin) {
          ...
          navigate(disciplineConfig?.routes?.dashboard || '/select-discipline');
        } else {
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="flex items-center justify-center sm:justify-between w-full gap-2">
          <div className="flex items-center space-x-2 sm:ml-[5%]">
            ...
            <Link 
              to={isAdminMode ? "/admin" : disciplineConfig?.routes?.dashboard || '/select-discipline'} 
          ...
          <div className="flex items-center space-x-2">
            ...
            {/* Profile Avatar */}
            <Link
              to={location.pathname.includes('/profile') ? disciplineConfig?.routes?.dashboard || '/select-discipline' : disciplineConfig?.routes?.profile || '/select-discipline'}
              className={`relative flex items-center justify-center w-10 h-10 rounded-full shadow cursor-pointer transition-colors border-2 border-black ${
