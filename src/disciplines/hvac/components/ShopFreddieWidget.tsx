...
import { useFreddieContext } from './ShopFreddieContext';
...
const getProactiveMessage = (page: string, t: any) => {
  switch (page) {
    ...
    default:
      return "Hey! I'm Freon Frankie, your AI HVAC assistant. Ask me anything about refrigeration, heat pumps, or EPA compliance.";
  }
};
...
const getChips = (page: string): string[] => {
  switch (page) {
    case 'MyShop':
      return ['Look up equipment specs for me', 'Check refrigerant requirements', 'How do I use My Shop?'];
    case 'MySpecSheets':
      return ['Help me document a service report', 'Review equipment specs with me', 'How do I use My Spec Sheets?'];
    case 'TechTalk':
      return ['Help me find a local HVAC supplier', 'Connect me with other HVAC techs', 'How do I use Tech Talk?'];
    case 'HvacSchool':
      return ['Help me prep for EPA 608', 'Explain a diagnostic procedure', 'How do I use HVAC School?'];
    default:
      return ['Help with a refrigerant question', 'Explain heat pump operation', 'How do I use this module?'];
  }
};
...
const ShopFreddieWidget = () => {
  ...
  const sendUserMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages(msgs => [...msgs, { sender: 'freddie', text: err.message || t('shopFreddie.errorContacting') }]);
    }
  };
  ...
  return (
    <>
      <button
        ...
        onClick={() => setOpen(o => !o)}
        aria-label="Open Freon Frankie AI Assistant"
      >
        ...
        <React.Fragment>
          <div className="fixed bottom-24 right-6 bg-white border-4 border-maineBlue rounded-xl shadow-xl w-80 z-50 flex flex-col max-h-[60vh]">
            <div className="flex justify-between items-center px-4 py-3 bg-blue-50 rounded-t-xl border-b-2 border-maineBlue">
              <span className="font-retro font-bold text-maineBlue">Freon Frankie</span>
              <button onClick={() => {
                setOpen(false);
                setMessages([]);
