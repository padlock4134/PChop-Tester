...
import WeeklyChallengeSystemModal from './WeeklyChallengeSystemModal';
...
export const WEEKLY_CHALLENGES = [
  {
    title: 'Airflow Audit',
    description: 'Complete a challenge involving airflow diagnostics.',
    criteria: (recipe: { ingredients: string[] }) => recipe.ingredients.some(i => ['airflow','static','duct'].includes(i.toLowerCase())),
    reward: { xp: 100, badge: 'Airflow Ace' },
  },
  {
    title: 'Refrigerant Ready',
    description: 'Submit work involving refrigerant handling.',
    criteria: (recipe: { ingredients: string[] }) => recipe.ingredients.some(i => ['refrigerant','charge','superheat'].includes(i.toLowerCase())),
    reward: { xp: 100, badge: 'Refrigerant Pro' },
  },
  {
    title: 'Thermostat Tune',
    description: 'Complete thermostat or controls tuning.',
    criteria: (recipe: { ingredients: string[] }) => recipe.ingredients.some(i => ['thermostat','control','setpoint'].includes(i.toLowerCase())),
    reward: { xp: 100, badge: 'Control Tuner' },
  },
  {
    title: 'Duct Detective',
    description: 'Log a duct leakage or balancing task.',
    criteria: (recipe: { ingredients: string[] }) => recipe.ingredients.some(i => ['duct','balance','leak'].includes(i.toLowerCase())),
    reward: { xp: 100, badge: 'Duct Detective' },
  },
  {
    title: 'Filter Focus',
    description: 'Complete a filtration/maintenance task.',
    criteria: (recipe: { ingredients: string[] }) => recipe.ingredients.some(i => ['filter','maintenance','iaq'].includes(i.toLowerCase())),
    reward: { xp: 100, badge: 'Filter Focus' },
  },
  {
    title: 'Cooling Cycle',
    description: 'Submit compressor/cycle diagnostic work.',
    criteria: (recipe: { ingredients: string[] }) => recipe.ingredients.some(i => ['compressor','cycle','cooling'].includes(i.toLowerCase())),
    reward: { xp: 100, badge: 'Cycle Specialist' },
  },
  {
    title: 'Heating Check',
    description: 'Complete a heating performance task.',
    criteria: (recipe: { ingredients: string[] }) => recipe.ingredients.some(i => ['heating','burner','heat exchanger'].includes(i.toLowerCase())),
    reward: { xp: 100, badge: 'Heat Specialist' },
  },
  {
    title: 'Load Logic',
    description: 'Log work tied to load calculation/planning.',
    criteria: (recipe: { ingredients: string[] }) => recipe.ingredients.some(i => ['load','btu','sizing'].includes(i.toLowerCase())),
    reward: { xp: 100, badge: 'Load Planner' },
  },
  {
    title: 'Ventilation Verify',
    description: 'Complete a ventilation verification task.',
    criteria: (recipe: { ingredients: string[] }) => recipe.ingredients.some(i => ['ventilation','fresh air','cfm'].includes(i.toLowerCase())),
    reward: { xp: 100, badge: 'Ventilation Verified' },
  },
  {
    title: 'Commissioning Day',
    description: 'Submit startup/commissioning checklist work.',
    criteria: (recipe: { ingredients: string[] }) => recipe.ingredients.some(i => ['startup','commission','checklist'].includes(i.toLowerCase())),
    reward: { xp: 100, badge: 'Commissioning Pro' },
  }
];
