...
const GlobalTestKitchen: React.FC<GlobalTestKitchenProps> = ({ showcaseRecipe }) => {
  ...
  const handleReport = async (sessionId?: string, reason?: string) => {
    if (!user) {
      alert(t('techTalk.globalTestKitchen.pleaseLogInReport'));
      return;
    }
    ...
    if (confirm(t('techTalk.globalTestKitchen.reportSession'))) {
      try {
        ...
        if (error) {
          ...
          alert(t('techTalk.globalTestKitchen.failedToReport'));
        } else {
          ...
          alert(t('techTalk.globalTestKitchen.thankYouReport'));
        }
      } catch (error) {
  ...
  const handleScheduleSession = async () => {
    if (!user) {
      alert(t('techTalk.globalTestKitchen.pleaseLogInSchedule'));
      return;
    }
    ...
    if (scheduledDishName.trim() && scheduledCuisine && scheduledDescription.trim() && scheduledDate && scheduledTime) {
      try {
        ...
        if (error) {
          ...
          alert(t('techTalk.globalTestKitchen.failedToSchedule').replace('{error}', error.message));
          return;
        }
        ...
        alert(t('techTalk.globalTestKitchen.failedToScheduleGeneric'));
      }
    }
  };
  ...
  const handleSaveSession = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        alert(t('techTalk.globalTestKitchen.failedToSaveVideo').replace('{error}', error.message || 'Unknown error'));
        setIsSaving(false);
        return;
      }
      ...
      alert(t('techTalk.globalTestKitchen.failedToSaveVideoGeneric'));
    } finally {
      setIsSaving(false);
      setSaveConfirmModalOpen(false);
      endRecordingSession();
    ...
    <div className="bg-white rounded-lg shadow-lg border-4 border-maineBlue overflow-hidden w-full min-h-[800px]">
      ...
      <div className="p-4">
        <p className="text-sm text-gray-600 text-center mb-4">
          {t('techTalk.globalTestKitchen.subtitle')}
        </p>
