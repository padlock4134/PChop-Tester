...
const hvacQuoteOfTheDay = {
  chef: 'Willis Carrier',
  quote: 'Comfort comes from mastering airflow, temperature, and detail.'
};

export function getChefQuoteOfTheDay() {
  return hvacQuoteOfTheDay;
}

export function getVideoQueriesForRecipe(recipe: Recipe): string[] {
  return [
    `how to ${recipe.name} HVAC tutorial`,
...
const MySpecSheets = () => {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from('user_collections').select('*').eq('user_id', user.id).eq('discipline', 'hvac')
      .then(({ data }) => {
  ...
  const handleCreateCollection = async () => {
    ...
    const { data, error } = await supabase.from('user_collections').insert({
      user_id: user?.id, discipline: 'hvac', name: newCollectionName.trim(), emoji: '📁', items: selectedRecipes
    }).select().single();
    if (!error && data) {
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:h-full lg:justify-center">
        <div className={`lg:w-[66.666%] bg-weatheredWhite rounded-xl shadow-lg border-4 border-maineBlue flex flex-col h-full lg:min-h-[620px] ${
          ...
          <div className="flex items-center justify-center p-6 pb-4">
            ...
            <h1 className="text-3xl font-retro text-maineBlue mb-0">{t('mySpecSheets.title')}</h1>
          </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-xl max-w-sm w-full mx-4 border-4 border-black" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex justify-around mb-4">
            <button 
              ...
              title={t('mySpecSheets.shareOnFacebook')}
            >
            ...
            <button 
              ...
              className="p-2 rounded-full hover:bg-blue-100"
              title={t('mySpecSheets.shareOnTwitter')}
            >
            ...
            <button 
              ...
              title={t('mySpecSheets.shareOnPinterest')}
            >
            ...
            <button 
              ...
              title={t('mySpecSheets.shareOnWhatsApp')}
            >
            ...
            <button 
              ...
              title={t('mySpecSheets.shareOnInstagram')}
            >
            ...
            <button 
              ...
              title={t('mySpecSheets.shareOnSlack')}
            >
      ...
      {showCreateCollectionModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => {
          ...
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full mx-4 border-4 border-black" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4">{t('mySpecSheets.createNewCollection')}</h3>
            
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                {t('mySpecSheets.collectionName')}
              </label>
              <input
                type="text"
                value={newCollectionName}
                onChange={(e) => setNewCollectionName(e.target.value)}
                placeholder={t('mySpecSheets.enterCollectionName')}
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-600">
                {t('mySpecSheets.selectedRecipes')} {selectedRecipes.length}
              </p>
            </div>
            ...
            <div className="flex justify-end gap-3">
              ...
              >
                {t('mySpecSheets.createCollection')}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full mx-4 border-4 border-black" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-600 mb-3">
                {t('mySpecSheets.recipesInCollection')} ({selectedCollection.recipes.length}):
              </p>
              ...
              <div className="h-40 overflow-y-scroll border border-gray-300 rounded p-2">
                ...
                {selectedCollection.recipes.length === 0 && (
                  <div className="py-4 text-center text-gray-500 text-sm italic">
                    {t('mySpecSheets.noRecipesInCollection')}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex justify-between">
              ...
              >
                {t('mySpecSheets.deleteCollection')}
              </button>
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-3">
          ...
          <div className="relative flex-1">
            <input
              type="text"
              placeholder={t('mySpecSheets.searchRecipes')}
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-1 w-full">
          ...
          >
            <span className="hidden sm:inline">{t('mySpecSheets.previous')}</span>
            <span className="sm:hidden">{t('mySpecSheets.prev')}</span>
          </button>
          <button
          ...
          >
            <span className="hidden sm:inline">{t('mySpecSheets.next')}</span>
            <span className="sm:hidden">{t('mySpecSheets.next')} →</span>
          </button>
          <button
            onClick={async () => {
            ...
            title={t('mySpecSheets.deleteRecipe')}
          >
            🗑️ {t('mySpecSheets.remove')}
          </button>
          <button
            onClick={() => {
                    ...
                    <div className="bg-seafoam/20 p-1.5 rounded-lg text-center border-2 border-seafoam">
                      ...
                      <ul className="list-disc pl-3 max-h-[50px] sm:max-h-[60px] overflow-y-auto text-left text-[10px] sm:text-xs space-y-0">
                        ...
                        {(filteredRecipes[currentIndex].ingredients?.length || 0) > 6 && (
                          <li className="text-gray-600 italic font-semibold">+{(filteredRecipes[currentIndex].ingredients?.length || 0) - 6} {t('mySpecSheets.more')}</li>
                        )}
                      </ul>
                    </div>
                    ...
                    <div className="bg-amber-50 p-1.5 rounded-lg text-center border-2 border-amber-300">
                      ...
                      <ul className="list-disc pl-3 max-h-[50px] sm:max-h-[60px] overflow-y-auto text-left text-[10px] sm:text-xs space-y-0">
                        ...
                        {(filteredRecipes[currentIndex].equipment?.length || 0) > 4 && (
                          <li className="text-gray-600 italic font-semibold">+{(filteredRecipes[currentIndex].equipment?.length || 0) - 4} {t('mySpecSheets.more')}</li>
                        )}
                      </ul>
                    </div>
                    ...
                    <div className="bg-green-50 p-1.5 rounded-lg text-center border-2 border-green-300">
                      ...
                      <div className="flex flex-wrap gap-1 justify-center max-h-[50px] sm:max-h-[60px] overflow-y-auto">
                        {filteredRecipes[currentIndex].healthTags?.slice(0, 4).map(tag => (
                          ...
                          <span className="text-xs text-gray-500">{t('mySpecSheets.noHealthTags')}</span>
                        )}
        ...
        <div className={`lg:w-[28.333%] lg:h-full ${
          ...
          <div className="bg-white rounded-lg shadow-lg border-4 border-maineBlue overflow-hidden w-full h-full lg:min-h-[620px] flex flex-col">
            <div className="p-4 bg-seafoam text-maineBlue font-retro text-center">
              <h3 className="text-xl">📚 {t('mySpecSheets.collectionsLibrary')}</h3>
            </div>
              ...
              <div className="mb-6">
                ...
                <div className="max-h-32 overflow-y-auto border border-gray-300 rounded p-2 bg-gray-50">
                  ...
                  {collections.length === 0 && (
                    <div className="py-4 text-center text-gray-500 text-sm italic">
                      {t('mySpecSheets.noCollectionsYet')}
                    </div>
                  )}
                </div>
              </div>
                  ...
                  <div className="w-full lg:w-1/2 h-1/2 lg:h-full bg-white border-b lg:border-b-0 lg:border-r-2 border-gray-300 rounded-t-lg lg:rounded-t-none lg:rounded-l-lg p-3 lg:p-4 overflow-y-auto flex flex-col">
                    ...
                    <div className="bg-white border-2 lg:border-4 border-green-500 rounded-lg p-2 lg:p-3 mb-2 shadow-sm flex-shrink-0">
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-2">
                        <h4 className="font-serif font-bold text-emerald-800 text-sm lg:text-xs">📊 {t('mySpecSheets.rubric')}</h4>
                      <select 
                      ...
                      <div className="grid grid-cols-2 gap-1 lg:gap-1">
                        ...
                        <div className="bg-white/60 p-2 lg:p-1 rounded border border-emerald-200">
                          <div className="text-xs font-medium text-emerald-900 mb-1">{t('mySpecSheets.technique')} (25)</div>
                          <select className="w-full text-xs border border-emerald-300 rounded px-1 py-1 lg:py-0.5 bg-white min-h-[32px] lg:min-h-0">
                          <option value="">{t('mySpecSheets.score')}</option>
                        ...
                        {/* Safety Score */}
                        <div className="bg-white/60 p-2 lg:p-1 rounded border border-emerald-200">
                          <div className="text-xs font-medium text-emerald-900 mb-1">{t('mySpecSheets.safety')} (25)</div>
                          <select className="w-full text-xs border border-emerald-300 rounded px-1 py-1 lg:py-0.5 bg-white min-h-[32px] lg:min-h-0">
                          <option value="">{t('mySpecSheets.score')}</option>
                          <option value="25">A (23-25)</option>
                          <option value="22">B (20-22)</option>
                        ...
                        {/* Consistency Score */}
                        <div className="bg-white/60 p-2 lg:p-1 rounded border border-emerald-200">
                          <div className="text-xs font-medium text-emerald-900 mb-1">{t('mySpecSheets.consistency')} (25)</div>
                          <select className="w-full text-xs border border-emerald-300 rounded px-1 py-1 lg:py-0.5 bg-white min-h-[32px] lg:min-h-0">
                          <option value="">{t('mySpecSheets.score')}</option>
                          <option value="25">A (23-25)</option>
                          <option value="22">B (20-22)</option>
                        ...
                        {/* Presentation Score */}
                        <div className="bg-white/60 p-2 lg:p-1 rounded border border-emerald-200">
                          <div className="text-xs font-medium text-emerald-900 mb-1">{t('mySpecSheets.presentation')} (25)</div>
                          <select className="w-full text-xs border border-emerald-300 rounded px-1 py-1 lg:py-0.5 bg-white min-h-[32px] lg:min-h-0">
                          <option value="">{t('mySpecSheets.score')}</option>
                          <option value="25">A (23-25)</option>
                          <option value="22">B (20-22)</option>
                      ...
                      <div className="mt-2 pt-2 border-t border-emerald-300 text-center py-2 lg:py-3">
                        <span className="text-base lg:text-xl font-bold text-red-600">
                          <span className="block lg:inline">{t('mySpecSheets.total')}: {(mockGrades as any)[students[currentStudentIndex].id]?.[assignments[currentAssignmentPage].id]?.total || '--'} / 100</span>
                          <span className="hidden lg:inline"> | </span>
                          <span className="block lg:inline">{t('mySpecSheets.grade')}: {(mockGrades as any)[students[currentStudentIndex].id]?.[assignments[currentAssignmentPage].id]?.grade || '--'}</span>
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-white border-2 lg:border-4 border-yellow-500 rounded-lg p-2 lg:p-3 mb-2 shadow-sm flex-shrink-0">
                      <h4 className="font-serif font-bold text-amber-800 mb-1 lg:mb-2 text-sm">💬 {t('mySpecSheets.feedback')}</h4>
                      
                      <textarea 
                        placeholder={t('mySpecSheets.feedbackPlaceholder')}
                    ...
                    <div className="bg-white border-2 lg:border-4 border-amber-700 rounded-lg p-2 lg:p-3 shadow-sm overflow-hidden flex flex-col flex-1 min-h-0">
                      <h4 className="font-serif font-bold text-yellow-800 mb-2 text-sm border-b border-yellow-300 pb-1">📝 {t('mySpecSheets.savedFeedbackNotes')}</h4>
                      ...
                      <div className="space-y-2 text-xs">
                        <div className="border-b border-gray-200 pb-2">
                          ...
                          <div className="text-gray-600">
                            "Great improvement on gauge handling and airflow adjustments. Your diffuser balance readings were much more consistent than last week. 
                            Focus on documenting each adjustment step in real time. Overall excellent progress."
                          </div>
                        </div>
                        ...
                        <div className="border-b border-gray-200 pb-2">
                          ...
                          <div className="text-gray-600">
                            "Good lockout/tagout awareness throughout the service demo. Work on double-checking disconnect status more consistently 
                            before opening the condenser panel. Your tool staging was excellent."
                          </div>
                        </div>
                        ...
                        <div className="border-b border-gray-200 pb-2">
                          ...
                          <div className="text-gray-600">
                            "First airflow assessment - showing natural talent! Remember to verify return static before 
                            final balancing for better accuracy. Practice the test-adjust-test cycle we discussed."
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col bg-white w-full h-full overflow-hidden rounded-lg border-4 border-maineBlue">
                      ...
                      <div className="w-full h-20 lg:h-24 bg-gray-100 flex items-center justify-center border-b-2 border-amber-300 flex-shrink-0">
                        <div className="text-center">
                          ...
                          <div className="text-xs font-bold text-amber-800">{assignments[currentAssignmentPage].week} {t('mySpecSheets.assignment')}</div>
                        </div>
                      </div>
                      ...
                      <div className="p-2 lg:p-3 bg-white text-center flex-1 overflow-y-auto flex flex-col">
                        ...
                        <div className="text-xs text-gray-600 mb-2 lg:mb-4">
                          <span className="block lg:inline">{t('mySpecSheets.due')}: {assignments[currentAssignmentPage].dueDate}</span>
                          <span className="hidden lg:inline"> | </span>
                        ...
                        <div className="space-y-1 lg:space-y-2 flex-shrink-0">
                          <div>
                            <div className="font-semibold mb-1 text-sm text-amber-800">{t('mySpecSheets.requiredTechniques')}</div>
                          ...
                          <div>
                            <div className="font-semibold mb-1 text-sm text-amber-800">{t('mySpecSheets.submission')}</div>
                            <div className="text-xs text-gray-700 leading-tight">
                          ...
                          <div>
                            <div className="font-semibold mb-1 text-sm text-amber-800">{t('mySpecSheets.objectives')}</div>
                            <div className="text-xs text-gray-700 leading-tight">
                        ...
                        <div className="text-center mt-2 flex-1 flex flex-col min-h-0">
                          <div className="flex flex-col lg:flex-row items-center justify-center gap-1 lg:gap-2 mb-2 flex-shrink-0">
                            <h4 className="font-serif font-semibold text-amber-800 text-sm">{students[currentStudentIndex].name} - {t('mySpecSheets.submission')}</h4>
                            <select 
                            ...
                            >
                              <option value="">{t('mySpecSheets.selectVideo')}</option>
                              <option value="airflow-balance-demo">Airflow Balancing Demo.mp4</option>
                              <option value="charging-technique">Charging Technique.mp4</option>
                              <option value="defrost-control-demo">Protein Cookery.mp4</option>
                          ...
                          <div className="bg-gray-900 rounded-lg overflow-hidden border border-amber-300 relative flex-1 min-h-[150px]">
                            <div className="h-full bg-gray-800 flex items-center justify-center">
                              <div className="text-center text-white p-2">
                                ...
                                <div className="text-xs text-gray-300 mt-1 hidden lg:block">{t('mySpecSheets.submittedVia')}</div>
                              </div>
                            </div>
                            <button 
              ...
              <div className="p-4 bg-gray-100">
                <div className="bg-black aspect-video flex items-center justify-center border-2 border-gray-400 rounded-lg overflow-hidden shadow-lg">
                  <div className="text-center text-white">
                    ...
                    <div className="text-sm text-gray-300">{t('mySpecSheets.clickToPlay')}</div>
                    <div className="text-xs text-gray-400 mt-2">{t('mySpecSheets.submittedVia')}</div>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-white rounded-lg shadow-2xl border-4 border-black max-w-md w-[90%] mx-4">
            ...
            <div className="p-6 text-center">
              ...
              <p className="font-bold text-maineBlue mb-4">
                {selectedVideoOption === 'airflow-balance-demo' && 'Airflow Balancing Demo.mp4'}
                {selectedVideoOption === 'charging-technique' && 'Charging Technique.mp4'}
                {selectedVideoOption === 'defrost-control-demo' && 'Protein Cookery.mp4'}
            ...
            <div className="bg-purple-100 border-b-4 border-purple-400 p-6">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                <div className="flex items-center gap-2">
                  <label className="text-purple-700 font-bold text-sm">{t('mySpecSheets.category')}:</label>
                  <select
                    value={videoFilter}
                  ...
                  >
                    <option value="all">{t('mySpecSheets.allVideos')}</option>
                    <option value="practice">{t('mySpecSheets.practiceSessions')}</option>
                    <option value="assignments">{t('mySpecSheets.assignmentSubmissions')}</option>
                    <option value="demos">{t('mySpecSheets.demoRecordings')}</option>
                  </select>
                </div>
                
                <div className="flex items-center gap-2">
                  <label className="text-purple-700 font-bold text-sm">{t('mySpecSheets.user')}:</label>
                  <select
                    value={userFilter}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {loadingVideos ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500 text-sm mt-2">{t('mySpecSheets.recordInTestKitchen')}</p>
                </div>
              ) : (
                        ...
                        <div className="flex items-center gap-4 flex-1">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-3 mt-1">
                              ...
                              {video.isPublic ? (
                                <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded font-bold">🌍 {t('mySpecSheets.public')}</span>
                              ) : (
                                <span className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded font-bold">🔒 {t('mySpecSheets.private')}</span>
                              )}
                            </div>
