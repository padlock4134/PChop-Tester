...
export const awardXP = async (userId: string, xpAmount: number, action: string) => {
  try {
    ...
    if (action.includes('complete')) {
      ...
      trackCompletion({ user_id: userId, discipline: 'manufacturing', module_type: action, module_id: action, started_at: startedAt, completed_at: new Date().toISOString() });
    }
