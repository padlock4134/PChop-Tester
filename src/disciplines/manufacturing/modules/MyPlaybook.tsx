...
const MyPlaybook = () => {
  ...
  useEffect(() => {
    if (!user) return;
    supabase.from('user_collections').select('*').eq('user_id', user.id).eq('discipline', 'manufacturing')
      .then(({ data }) => {
        if (data) setCollections(data.map(r => ({ id: r.id, name: r.name, emoji: r.emoji, processes: r.items as string[] })));
      });
  }, [user]);
  ...
  const handleCreateCollection = async () => {
    ...
    const { data, error } = await supabase.from('user_collections').insert({
      user_id: user?.id, discipline: 'manufacturing', name: newCollectionName.trim(), emoji: '📁', items: selectedProcesses
    }).select().single();
    if (!error && data) {
      setCollections(prev => [...prev, { id: data.id, name: data.name, emoji: data.emoji, processes: data.items as string[] }]);
    }
    setNewCollectionName(''); setSelectedProcesses([]); setShowCreateCollectionModal(false);
  };
      ...
      <div className="flex flex-col lg:flex-row gap-6 lg:h-full lg:justify-center">
        <div className={`lg:w-[66.666%] bg-weatheredWhite rounded-xl shadow-lg border-4 border-maineBlue flex flex-col h-full lg:min-h-[620px] ${
          ...
          <div className="flex items-center justify-center p-6 pb-4">
            ...
            <h1 className="text-3xl font-retro text-maineBlue mb-0">{t('myPlaybook.title')}</h1>
          </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-xl max-w-sm w-full mx-4 border-4 border-black" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex justify-around mb-4">
            <button 
              ...
              title={t('myPlaybook.shareOnFacebook')}
            >
            ...
            <button 
              ...
              className="p-2 rounded-full hover:bg-blue-100"
              title={t('myPlaybook.shareOnTwitter')}
            >
            ...
            <button 
              ...
              title={t('myPlaybook.shareOnPinterest')}
            >
            ...
            <button 
              ...
              title={t('myPlaybook.shareOnWhatsApp')}
            >
            ...
            <button 
              ...
              title={t('myPlaybook.shareOnInstagram')}
            >
            ...
            <button 
              ...
              title={t('myPlaybook.shareOnSlack')}
            >
      ...
      {showCreateCollectionModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => {
          ...
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full mx-4 border-4 border-black" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold mb-4">{t('myPlaybook.createNewCollection')}</h3>
            
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                {t('myPlaybook.collectionName')}
              </label>
              <input
                type="text"
                value={newCollectionName}
                onChange={(e) => setNewCollectionName(e.target.value)}
                placeholder={t('myPlaybook.enterCollectionName')}
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-3">
          ...
          <div className="relative flex-1">
            <input
              type="text"
              placeholder={t('myPlaybook.searchRecipes')}
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-1 w-full">
          ...
          >
            <span className="hidden sm:inline">{t('myPlaybook.previous')}</span>
          ...
          <button
            ...
            className={`flex-1 px-2 py-2 rounded border border-black text-xs sm:text-sm font-bold transition-colors text-center ${currentIndex === filteredProcesses.length - 1 ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-seafoam text-maineBlue hover:bg-maineBlue hover:text-seafoam'}`}
          >
          ...
          <button
            ...
            className={`flex-1 px-2 py-2 rounded border border-black text-xs sm:text-sm font-bold transition-colors text-center ${filteredProcesses.length === 0 ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-lobsterRed text-weatheredWhite hover:bg-red-800'}`}
          ...
          <button
            ...
            disabled={filteredProcesses.length === 0}
            className={`flex-1 px-2 py-2 rounded border border-black text-xs sm:text-sm font-bold transition-colors text-center ${filteredProcesses.length === 0 ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-maineBlue text-seafoam hover:bg-seafoam hover:text-maineBlue'}`}
          >
            Share
          </button>
        </div>
          ...
          >
            ...
            >
              ...
              <div className="absolute inset-0 bg-white p-2 sm:p-3 rounded-lg shadow-lg border-4 border-maineBlue flex flex-col items-center [backface-visibility:hidden]">
                ...
                <h3 className="text-sm sm:text-base md:text-lg font-bold mb-1 text-center text-maineBlue">{filteredProcesses[currentIndex].name}</h3>
                <div className="flex-1 flex flex-col justify-center w-full px-2">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 w-full">
                    ...
                    <div className="bg-amber-50 p-1.5 rounded-lg text-center border-2 border-amber-300">
                      <h4 className="font-bold mb-0.5 text-xs sm:text-sm text-amber-900">🔪 {t('myPlaybook.equipment')}</h4>
                      <ul className="list-disc pl-3 max-h-[50px] sm:max-h-[60px] overflow-y-auto text-left text-[10px] sm:text-xs space-y-0">
                    ...
                    <div className="bg-green-50 p-1.5 rounded-lg text-center border-2 border-green-300">
                      <h4 className="font-bold mb-0.5 text-xs sm:text-sm text-green-900">🥗 {t('myPlaybook.healthTags')}</h4>
                  ...
                  <div className="w-full lg:w-1/2 h-1/2 lg:h-full bg-white border-b lg:border-b-0 lg:border-r-2 border-gray-300 rounded-t-lg lg:rounded-t-none lg:rounded-l-lg p-3 lg:p-4 overflow-y-auto flex flex-col">
                    ...
                    <div className="bg-white border-2 lg:border-4 border-green-500 rounded-lg p-2 lg:p-3 mb-2 shadow-sm flex-shrink-0">
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-2">
                        <h4 className="font-serif font-bold text-emerald-800 text-sm lg:text-xs">📊 {t('myPlaybook.rubric')}</h4>
                      <select 
                      ...
                      <div className="grid grid-cols-2 gap-1 lg:gap-1">
                        ...
                        {/* Safety Score */}
                        <div className="bg-white/60 p-2 lg:p-1 rounded border border-emerald-200">
                          <div className="text-xs font-medium text-emerald-900 mb-1">{t('myPlaybook.safety')} (25)</div>
                          <select className="w-full text-xs border border-emerald-300 rounded px-1 py-1 lg:py-0.5 bg-white min-h-[32px] lg:min-h-0">
                          <option value="">{t('myPlaybook.score')}</option>
                          <option value="25">A (23-25)</option>
                          <option value="22">B (20-22)</option>
                      ...
                      <div className="mt-2 pt-2 border-t border-emerald-300 text-center py-2 lg:py-3">
                        <span className="text-base lg:text-xl font-bold text-red-600">
                          <span className="block lg:inline">{t('myPlaybook.total')}: {(mockGrades as any)[students[currentStudentIndex].id]?.[assignments[currentAssignmentPage].id]?.total || '--'} / 100</span>
                          <span className="hidden lg:inline"> | </span>
                          <span className="block lg:inline">{t('myPlaybook.grade')}: {(mockGrades as any)[students[currentStudentIndex].id]?.[assignments[currentAssignmentPage].id]?.grade || '--'}</span>
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-white border-2 lg:border-4 border-yellow-500 rounded-lg p-2 lg:p-3 mb-2 shadow-sm flex-shrink-0">
                      <h4 className="font-serif font-bold text-amber-800 mb-1 lg:mb-2 text-sm">💬 {t('myPlaybook.feedback')}</h4>
                      
                      <textarea 
                        placeholder={t('myPlaybook.feedbackPlaceholder')}
                    ...
                    <div className="bg-white border-2 lg:border-4 border-amber-700 rounded-lg p-2 lg:p-3 shadow-sm overflow-hidden flex flex-col flex-1 min-h-0">
                      <h4 className="font-serif font-bold text-yellow-800 mb-2 text-sm border-b border-yellow-300 pb-1">📝 {t('myPlaybook.savedFeedbackNotes')}</h4>
                    ...
                    <div className="flex flex-col bg-white w-full h-full overflow-hidden rounded-lg border-4 border-maineBlue">
                      ...
                      <div className="w-full h-20 lg:h-24 bg-gray-100 flex items-center justify-center border-b-2 border-amber-300 flex-shrink-0">
                        <div className="text-center">
                          ...
                          <div className="text-xs font-bold text-amber-800">{assignments[currentAssignmentPage].week} {t('myPlaybook.assignment')}</div>
                        </div>
                      </div>
                      ...
                      <div className="p-2 lg:p-3 bg-white text-center flex-1 overflow-y-auto flex flex-col">
                        ...
                        <div className="text-xs text-gray-600 mb-2 lg:mb-4">
                          <span className="block lg:inline">{t('myPlaybook.due')}: {assignments[currentAssignmentPage].dueDate}</span>
                          <span className="hidden lg:inline"> | </span>
                        ...
                        <div className="space-y-1 lg:space-y-2 flex-shrink-0">
                          <div>
                            <div className="font-semibold mb-1 text-sm text-amber-800">{t('myPlaybook.requiredTechniques')}</div>
                          ...
                          <div>
                            <div className="font-semibold mb-1 text-sm text-amber-800">{t('myPlaybook.submission')}</div>
                            <div className="text-xs text-gray-700 leading-tight">
                          ...
                          <div>
                            <div className="font-semibold mb-1 text-sm text-amber-800">{t('myPlaybook.objectives')}</div>
                            <div className="text-xs text-gray-700 leading-tight">
                        ...
                        <div className="text-center mt-2 flex-1 flex flex-col min-h-0">
                          <div className="flex flex-col lg:flex-row items-center justify-center gap-1 lg:gap-2 mb-2 flex-shrink-0">
                            <h4 className="font-serif font-semibold text-amber-800 text-sm">{students[currentStudentIndex].name} - {t('myPlaybook.submission')}</h4>
                            <select 
                          ...
                          <div className="bg-gray-900 rounded-lg overflow-hidden border border-amber-300 relative flex-1 min-h-[150px]">
                            <div className="h-full bg-gray-800 flex items-center justify-center">
                              <div className="text-center text-white p-2">
                                ...
                                <div className="text-xs text-gray-300 mt-1 hidden lg:block">{t('myPlaybook.submittedVia')}</div>
                              </div>
                            </div>
                            <button 
              ...
              <div className="p-4 bg-gray-100">
                <div className="bg-black aspect-video flex items-center justify-center border-2 border-gray-400 rounded-lg overflow-hidden shadow-lg">
                  <div className="text-center text-white">
                    ...
                    <div className="text-sm text-gray-300">{t('myPlaybook.clickToPlay')}</div>
                    <div className="text-xs text-gray-400 mt-2">{t('myPlaybook.submittedVia')}</div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-purple-100 border-b-4 border-purple-400 p-6">
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                <div className="flex items-center gap-2">
                  <label className="text-purple-700 font-bold text-sm">{t('myPlaybook.category')}:</label>
                  <select
                    value={videoFilter}
                ...
                <div className="flex items-center gap-2">
                  <label className="text-purple-700 font-bold text-sm">{t('myPlaybook.user')}:</label>
                  <select
                    value={userFilter}
            ...
            <div className="flex-1 overflow-y-auto p-6">
              {loadingVideos ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">{t('myPlaybook.loadingYourVideos')}</p>
                </div>
                        ...
                        <div className="flex items-center gap-4 flex-1">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center gap-3 mt-1">
                              ...
                              {video.isPublic ? (
                                <span className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded font-bold">🌍 {t('myPlaybook.public')}</span>
                              ) : (
                                <span className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded font-bold">🔒 {t('myPlaybook.private')}</span>
                              )}
                            </div>
