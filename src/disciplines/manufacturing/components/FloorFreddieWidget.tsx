...
const getProactiveMessage = (page: string, t: any) => {
  switch (page) {
    case 'MyKitchen':
      return t('floorFreddie.welcomeMyKitchen');
    case 'MyCookBook':
      return t('floorFreddie.welcomeMyCookbook');
    case 'ChefsCorner':
      return t('floorFreddie.welcomeChefsCorner');
    case 'CulinarySchool':
      return t('floorFreddie.welcomeCulinarySchool');
    default:
      return t('floorFreddie.welcomeDefault');
  }
};
...
const FloorFreddieWidget = () => {
  ...
  const sendUserMessage = async (text: string) => {
    ...
    try {
      ...
      setMessages(msgs => [...msgs, { sender: 'freddie', text: err.message || t('floorFreddie.errorContacting') }]);
    }
  };
        ...
        <React.Fragment>
          <div className="fixed bottom-24 right-6 bg-white border-4 border-maineBlue rounded-xl shadow-xl w-80 z-50 flex flex-col max-h-[60vh]">
            <div className="flex justify-between items-center px-4 py-3 bg-blue-50 rounded-t-xl border-b-2 border-maineBlue">
              <span className="font-retro font-bold text-maineBlue">Button Pusher Max</span>
              <button onClick={() => {
                setOpen(false);
                setMessages([]);
...
export default FloorFreddieWidget;
